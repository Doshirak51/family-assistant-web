...
function showSetup() {
  ...
  else { const form = el("form", "form"); const label = el("label", "", "Как назовём ваше пространство?"); const name = el("input", "input"); name.value = "Дарина ❤️ Кристина"; name.maxLength = 80; label.append(name); form.append(label); const create = button("Создать семейное пространство"); form.append(create); form.addEventListener("submit", async (event) => { event.preventDefault(); create.disabled = true; const { error } = await sb.rpc("family_web_create_household", { p_name: name.value.trim() }); if (error) { alert.textContent = errorText(error); alert.classList.remove("hidden"); create.disabled = false; } else await boot(); }); panel.append(form); } section.append(panel); app.append(section);
}
...
function submitForm(d, form, save) { const alert=el("p","alert hidden");form.append(alert);form.addEventListener("submit",async(e)=>{e.preventDefault();const btn=form.querySelector("button[type=submit]");btn.disabled=true;try{await save();d.close();await render()}catch(error){alert.textContent=errorText(error);alert.classList.remove("hidden");btn.disabled=false}});d.append(form);d.showModal(); }
